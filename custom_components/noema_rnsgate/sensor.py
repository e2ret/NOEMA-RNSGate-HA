...
async def async_setup_entry(
    ...
    entities = [
        NOEMASensor(coordinator, entry, "ip_address", "IP Address", None, "mdi:ip-network",
                    None, None,
                    lambda d: d.get("system", {}).get("ip")),
        NOEMASensor(coordinator, entry, "cpu_usage", "CPU Usage", PERCENTAGE, "mdi:cpu-64-bit",
                    None, SensorStateClass.MEASUREMENT,
                    lambda d: d.get("system", {}).get("cpu")),
        NOEMASensor(coordinator, entry, "ram_usage", "RAM Usage", PERCENTAGE, "mdi:memory",
                    None, SensorStateClass.MEASUREMENT,
                    lambda d: round(d.get("system", {}).get("ram_used", 0) / d.get("system", {}).get("ram_total", 1) * 100, 1) if d.get("system", {}).get("ram_total") else None),
        NOEMASensor(coordinator, entry, "disk_usage", "Disk Usage", PERCENTAGE, "mdi:harddisk",
                    None, SensorStateClass.MEASUREMENT,
                    lambda d: round(d.get("system", {}).get("disk_used", 0) / d.get("system", {}).get("disk_total", 1) * 100, 1) if d.get("system", {}).get("disk_total") else None),
        NOEMASensor(coordinator, entry, "cpu_temperature", "CPU Temperature", UnitOfTemperature.CELSIUS, "mdi:thermometer",
        ...
        NOEMASensor(coordinator, entry, "lxmf_bridge_address", "LXMF Bridge Address", None, "mdi:identifier",
                    None, None,
